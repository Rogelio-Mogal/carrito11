...
<div class="grid lg:grid-cols-12 md:grid-cols-12 sm:grid-cols-12 gap-2">
    <div class="sm:col-span-12 lg:col-span-1 md:col-span-4">
        ...
        <div class="input-group">
            <select id="precio" name="precio" style="height: 400px !important;"
                ...
                <option value="" disabled
                    @if ($metodo == 'create' || old('precio', isset($precio) ? $precio->precio : '') == '') selected @endif>
                    -- PRECIO --
                </option>
        ...
        <p class="msj-precio mt-2 text-xs text-red-600 dark:text-red-400 hidden"><span class="font-medium">Error.</span></p> 
    </div>
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            $('select[name="precio"]').on('change', function() {
                var productoId = $("#precio :selected").val();
                $('#tipo').val(productoId);
            });

            // FUNCION PARA COMPARARA LOS RANGOS CON LA BD
            function preciosBd(inicio, fin, precio, callback) {
                const ajaxData = {
                    "_token": "{{ csrf_token() }}",
                    desde: inicio,
                    hasta: fin,
                            ...
                            if (!duplicado) {
                                ...
                                if (rangos.length === 1 || !ultimoRango || resultado ===
                                    ...
                                    html += '<input type="hidden" name="hasta[]" value="' +
                                        ...
                                        '"  />';
                                    html +=
                                        '<input type="hidden" name="tipo_precio[]" value="1"  />';
                                    html += desde;
                                    html += '</td>';
                                    html += '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-black">';
                                    html += hasta;
                                    ...
                                    html += '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-black">';
                                    html +=
                                        '<input type="number"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" min="1" max="100" step="any" required name="porcentaje_publico[]" value="' +
                                        publico + '"  />';
                                    html += '</td>';
                                    html += '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-black">';
                                    html +=
                                        '<input type="number"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" min="1" max="100" step="any" required name="porcentaje_medio[]" value="' +
                                        medio + '"  />';
                                    html += '</td>';
                                    html += '<td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-black">';
                                    html +=
                                        '<input type="number"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" min="1" max="100" step="any" required name="porcentaje_mayoreo[]" value="' +
                                        mayoreo + '"  />';
                                    html += '</td>';
