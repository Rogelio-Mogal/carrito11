@extends('layouts.app', [
    'breadcrumb' => [
        ...
        [
            ...
            'url' => route('admin.venta.credito.index'),
        ],
        [
            'name' => 'name',
    ...
    <div class="shadow-md rounded-lg p-4 dark:bg-gray-800">
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 lg:col-span-12 space-y-2">
                ...
                <!-- Módulo de Abono -->
                <div class="mb-4">
                    <h3 class="font-bold text-blue-600">REGISTRAR ABONO -VENTA A CRÉDITO-</h3>

                    <form id="form-abono" method="POST" action="{{ route('admin.venta.credito.abono.store') }}"
                        ...
                        <div id="abono-por-venta" class="sm:col-span-12 md:col-span-5 lg:col-span-5">
                            <label class="block font-semibold text-sm mb-2">Seleccionar venta:</label>
                            <select name="venta_credito_id" class="form-select w-full">
                                @foreach ($ventasCredito as $vc)
                                    <option value="{{ $vc->id }}">
                                        Venta: {{ $vc->venta->folio }} -
                                        Monto: ${{ number_format($vc->monto_credito, 2) }} -
                                        Saldo: ${{ number_format($vc->saldo_actual, 2) }} -
                ...
                <div class="mb-4">
                    <h3 class="font-bold text-blue-600">LISTADO DE VENTAS A CRÉDITO (Pendientes)</h3>
                    <table id="ventas-credito" class="table table-striped w-full">
                        ...
                        <tbody>
                            @foreach ($ventasCredito as $vc)
                                <tr>
                                    <td>{{ $vc->venta->folio ?? '-' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($vc->venta->fecha)->format('d/m/Y H:i:s') ?? '-' }}</td>
                                    <td>${{ number_format($vc->monto_credito, 2) }}</td>
                                    <td>${{ number_format($vc->saldo_actual, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
