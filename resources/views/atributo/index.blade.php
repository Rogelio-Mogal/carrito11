...
<div class="shadow-md rounded-lg p-4 dark:bg-gray-800">
    <div class="grid grid-cols-1 lg:grid-cols-12 md:grid-cols-12 sm:grid-cols-12 gap-4">
        <div class="sm:col-span-12 lg:col-span-12 md:col-span-12">
            ...
            <table id="atributo" class="table table-striped" style="width:100%">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Nombre</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#atributo').on('click', '.delete-item', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.atributos.destroy', ':id') }}"
                            .replace(':id', id),
                        type: 'POST',
                        data: {
                            "_token": "{{ csrf_token() }}",
        ...
        $('#atributo').on('click', '.activa-item', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Solicitud AJAX para eliminar el elemento
                    $.ajax({
                        url: "{{ route('admin.atributos.update', ':id') }}"
                            .replace(':id', id),
                        type: 'PUT',
                        data: {
                            "_token": "{{ csrf_token() }}",
    ...
    function atributo() {
        const postData = {
            ...
            origen: 'atributo.index',
        };
        var editUrl = "{{ route('admin.atributos.edit', ':id') }}";

        // Inicializar DataTable
        tblAtributo = $('#atributo').DataTable({
            "language": {
