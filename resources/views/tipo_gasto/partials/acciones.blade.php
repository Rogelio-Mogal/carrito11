@if ($item->activo == 1)
    ...
    <a href="{{ route('admin.tipo.gasto.destroy', $item->id) }}" data-popover-target="eliminar{{ $item->id }}"
...
@else
    <a href="{{ route('admin.tipo.gasto.edit', $item->id) }}" data-popover-target="activar{{ $item->id }}"
        data-popover-placement="left" data-id="{{ $item->id }}"
